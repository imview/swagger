...
public class StringUtil {
    ...
    public static String stripStart(String str, String stripChars) {
        ...
        int start = 0;
        if (stripChars == null) {
            while ((start != strLen) && (Character.isWhitespace(str.charAt(start)))) start++;
        }

        if (stripChars.length() == 0)
            return str;
        while ((start != strLen) && (stripChars.indexOf(str.charAt(start)) != -1)) start++;
    ...
    public static List stringToList(String PhoneRcvAddressStr) {
        List rcvAddressList = new ArrayList();
        String[] rcvAddress = PhoneRcvAddressStr.split(",");
        for (int i = 0; i < rcvAddress.length; i++) {
            rcvAddressList.add(rcvAddress[i]);
        }
        return rcvAddressList;
    }
}